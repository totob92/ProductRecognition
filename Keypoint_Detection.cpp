...
/*std::vector<std::vector<cv::DMatch>> DBSCAN_Match(cv::Mat image, std::vector<cv::DMatch> *matches, float eps, int minPts)
{
	...
	for (int i = 0; i < noKeys; i++)
	{
		if (!visited[i])
		{
			//Mark P as visited
			visited[i] = true;
			neighborPts = regionQueryMatches(matches, &matches->at(i), eps);
			if (neighborPts.size() < minPts)
				//Mark P as Noise
				noise.push_back(i);
			else
			{
				...
				clusters.push_back(std::vector<cv::DMatch>());
				//expand cluster
				// add P to cluster c
				clustered[i] = true;
				clusters[c].push_back(matches->at(i));
				//for each point P' in neighborPts
				for (int j = 0; j < neighborPts.size(); j++)
				{
					//if P' is not visited
					...
					{
						//Mark P' as visited
						neighborPts_ = regionQueryMatches(matches, &matches->at(neighborPts[j]), eps);
						visited[neighborPts[j]] = true;
					...
					// add P' to cluster c
					if (!clustered[neighborPts[j]])
						clusters[c].push_back(matches->at(neighborPts[j]));
					clustered[neighborPts[j]] = true;
				}
				c++;
			}
...
/*std::vector<int> regionQueryMatches(std::vector<cv::DMatch> *matches, cv::DMatch *match, float eps){
